...
targeting an IP accelerator and DNA `F200` architecture.
"""
##############################################################################
...
# ---------------------------------------------
# Although PyTorch only supports NCHW layout, for performance reasons it is
...
################################################################################
# Deploy and run for DNA accelerator
# ----------------------------------
...
with mera.TVMDeployer(out_dir, overwrite=True) as deployer:
    ...
    deploy_int = deployer.deploy(model, mera_platform=mera_platform, target=Target.InterpreterHw)
    ...
    # Compile for accelerator IP
    print(f'Deploying resnet50 for {target}...')
    deploy = deployer.deploy(model, mera_platform=mera_platform, target=target, host_arch=host_arch)
...
# Toggle this to enable running on device
...
# the elapsed latency and other metrics. For "IP" target, this is a 
# real measurement in microseconds of different metrics throughout the
# execution. We take the `elapsed_latency` metric which measures the time
# taken overall and we collect them all to report to the user.
if RUN_ON_IP:
    total_latency = sum(x.get('elapsed_latency', 0) for x in run_res.get_runtime_metrics()) / 1000
    print(f"IP total latency: {total_latency} ms")
