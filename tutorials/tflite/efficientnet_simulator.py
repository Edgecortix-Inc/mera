...
"""
EfficientNet Lite - Simulator
=============================

 This tutorial shows how to compile and run quantized TFLite models (EfficientNet) using MERA.
 We deploy and run the models using Simulator target and DNA `F200` architecture.
...
# -----------------------------
# We need to import the MERA environment. Additionally, we provide a helper function
...
# ------------------------
# First of all we create a MERA deployment project under 'deploy_effnet_liteX'
...
# with model_name
#
# Lastly deploy the model, targeting "Simulator" target. We can specify what
# clock frequency to use inside the simulator which will improve the quality
# of the reported latency metrics after the run.
#
# We can also specify `host_arch` to be "arm", to cross-compile the host code
# to run on a 64-bit ARM host.
# The compiled artifacts will be stored in `<output_dir>/result/`.
def mera_compile(tflite_filename, image_path, platform, host_arch, output_dir, sim_frequency):
    ...
    with mera.TVMDeployer(output_dir, overwrite=True) as deployer:
        ...
        b_cfg = {
            'sim_freq_mhz' : sim_frequency
        }
        deployer.deploy(model, mera_platform=platform, build_config=b_cfg, target=Target.Simulator, host_arch=host_arch)
    return input_data
...
# We then launch deployments targeting `Simulator` for the `DNAF200L0003` MERA platform
# with their own deployment directory for efficientnet-lite1 and efficientnet-lite4.
# Our host arch is `x86` since we are going to run it in host simulator.
# We set a sim frequency value of `300MHz` for getting more accurate latency metric reporting.
#
...
input_data_eflite1 = mera_compile("data/effnet-lite1.tflite", image_path, platform, host_arch, output_dir_lite1, SIM_FREQ)
input_data_eflite4 = mera_compile("data/effnet-lite4.tflite", image_path, platform, host_arch, output_dir_lite4, SIM_FREQ)
...
# metrics about the simulation that has run. From that object we get the `sim_time_us`
...
def get_total_latency_ms(run_result):
    ...
    total_us = sum([x.get('sim_time_us', 0) for x in metrics])
...
print(f"Optimized inference latency efficient net lite 1 (Simulator {SIM_FREQ} MHz): {get_total_latency_ms(mera_runner_lite1)} ms")

mera_runner_lite4 = ip_lite4.get_runner().set_input(input_data_eflite4).run()
mera_result_lite4 = mera_runner_lite4.get_outputs()
print(f"Optimized inference latency efficient net lite 4 (Simulator {SIM_FREQ} MHz): {get_total_latency_ms(mera_runner_lite4)} ms")
