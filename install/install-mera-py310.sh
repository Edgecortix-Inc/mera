...
sudo apt update && sudo apt install llvm-14 libgomp1 ocl-icd-libopencl1 software-properties-common \
    libgoogle-glog0v5 libboost-graph-dev virtualenv wget build-essential
sudo add-apt-repository ppa:ubuntu-toolchain-r/test
sudo apt update
sudo apt install libstdc++6

# Create virtual envinronment with Python 3.10
MERA_VENV=mera-env
virtualenv -p python3.10 $MERA_VENV
